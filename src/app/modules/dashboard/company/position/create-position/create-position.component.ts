...
export class CreatePositionComponent implements OnInit{
  ...
  combinedJobsAndPositions$ = combineLatest([this.jobFamilyList$, this.positions$])
  ...
  addPosition(form: FormGroup){
    ...
    if (this.positionService.action === 'view'){
      this.positionService.editPosition(this.positionService.selectedPosition.id, form.getRawValue()).pipe(
      ...
      this.positionService.createPosition(form.getRawValue())
        .pipe(
          tap(() => this.router.navigate(
            [NavigationPaths.BACK],
            { relativeTo: this.route }
  ...
  addDataToForm(){
    ...
    this.positionFormConfig.find(r => {
      ...
      if (r.key == 'directReportingLineId' || r.key == 'functionalReportingLineId'){
        r.data  = positions
      }
    })
    if (Object.keys(this.positionService.selectedPosition).length) {
      Object.keys(this.positionService.selectedPosition).forEach((key:any) => {
          if (this.form.controls[key]){
            this.form.controls[key].setValue(this.positionService.selectedPosition[key]);
          }
        this.positionFormConfig.forEach(select => {
          if (select.data) {
            select.data.find((data:any) => {
              if (data.value === this.positionService.selectedPosition[key]){
  ...
  formValueChanged(formValue:any){
    if (formValue.key == 'jobFamilyId'){
      this.positionService.getJobProfileList(formValue.value).subscribe(jobProfiles => {
        this.jobProfiles = jobProfiles.map((level:any) => ({displayName: level.name, value: level.id})),
          this.positionFormConfig.find(r => {
