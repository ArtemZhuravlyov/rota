...
export class CreateGroupFunctionDivisionComponent implements OnInit{
  ...
  addDivision(form: FormGroup) {
    if (this.divisionService.action == 'view'){
      ...
      this.divisionService.editDivision(this.divisionService.selectedDivision.id, requestBody).pipe(
      ...
      this.divisionService.createDivision(form.getRawValue())
        .pipe(
          tap(() => this.router.navigate(
            [NavigationPaths.BACK],
            { relativeTo: this.route }
    ...
    if (Object.keys(this.divisionService.selectedDivision).length) {
        Object.keys(this.divisionService.selectedDivision).forEach((key:any) => {
            if (this.form.controls[key]){
              this.form.controls[key].setValue(this.divisionService.selectedDivision[key])
            }
            this.groupFunctionDivisionFormConfig.forEach(select => {
              if (select.data) {
                select.data.find((data:any) => {
                  if (data.value === this.divisionService.selectedDivision[key]){
