...
import { createEmployeeFormConfig } from '@app/modules/dashboard/employee/create-employee/configs/create-employee-form-config';
  ...
  ngOnInit(): void {
    ...
    this.companyService
      ...
      .pipe(
        tap(
          countries =>
            (this.createEmployeeFormConfig[9].data = countries.map(
              country => ({
    ...
    this.settingsService
      ...
      .pipe(
        tap(
          results =>
            (this.createEmployeeFormConfig[4].data = results.map(
    ...
    this.settingsService
      ...
      .pipe(
        tap(
          results =>
            (this.createEmployeeFormConfig[5].data = results.map(
              res => ({ displayName: res.name, value: res.id })
            ))
        )
      )
