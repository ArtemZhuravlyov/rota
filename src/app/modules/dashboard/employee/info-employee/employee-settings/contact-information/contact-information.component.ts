...
import {
  emergencyContactFormFields, nextOfKinFormFields,
  residentialAddressFormFields
} from "@app/modules/dashboard/employee/info-employee/employee-settings/contact-information/configs/contact-information-form.config";
...
export class ContactInformationComponent {
  ...
  ngOnInit(): void {
    ...
    this.companyService.getCountry().pipe(
      tap(countries =>
        this.residentialAddressFormFields[2].data = countries.map(country =>
          ({ displayName: country.name, value: country.id, icon: country.countryFlag })
        )
      )
    ).subscribe();

    this.settingsService.getRelationshipType(this.authService.getCurrentUser().userId).pipe(
      tap(results =>
    ...
    this.companyService.getCountry().pipe(
      tap(countries =>
        this.residentialAddressFormFields[1].data = countries.map(country =>
          ({ displayName: country.twoLetterIsoCode, value: country.id })
        )
      )
    ).subscribe();

    this.stateRegionService.getStateRegionList(PAGE_SIZE, PAGE_INDEX).pipe(
      tap(states => {
        this.residentialAddressFormFields[1].data = states.stateRegions.map(state =>
          ({ displayName: state.name, value: state.id})
        );
      })
    ).subscribe();
  }
